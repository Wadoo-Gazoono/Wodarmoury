...
public class BoomShotItem extends AbstractBoomShotItem implements IAnimatable, ISyncable {
    ...
    public BoomShotItem(Properties properties) {
        super(properties);
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        for(LivingEntity livingentity : player.level.getEntitiesOfClass(LivingEntity.class, player.getBoundingBox().inflate(3d, 0.2D, 3d))) {
            if(livingentity != player) {
                if(player.getItemBySlot(EquipmentSlot.OFFHAND).is(WAItemRegister.BOOMSHOT.get()) && player.getItemBySlot(EquipmentSlot.MAINHAND).is(WAItemRegister.BOOMSHOT.get())){
                    livingentity.hurt(DamageSource.mobAttack(player), 10.0f);
                }
                else {
        ...
        if(player.getItemBySlot(EquipmentSlot.OFFHAND).is(WAItemRegister.BOOMSHOT.get()) && player.getItemBySlot(EquipmentSlot.MAINHAND).is(WAItemRegister.BOOMSHOT.get())){
            player.setDeltaMovement(viewVec.multiply(-1.8f, -1.8f, -1.8f));
            player.getCooldowns().addCooldown(this, 60);

        }
        else {
            player.setDeltaMovement(viewVec.multiply(-1.2f, -1.2f, -1.2f));
            player.getCooldowns().addCooldown(this, 30);
        ...
        if (!level.isClientSide) {
            ...
            final int id2 = GeckoLibUtil.guaranteeIDForStack(player.getItemBySlot(EquipmentSlot.OFFHAND), (ServerLevel) level);

            final PacketDistributor.PacketTarget target = PacketDistributor.TRACKING_ENTITY_AND_SELF
                    .with(() -> player);
            GeckoLibNetwork.syncAnimation(target, this, id, 1);
            if(player.getItemBySlot(EquipmentSlot.OFFHAND).is(WAItemRegister.BOOMSHOT.get()) && player.getItemBySlot(EquipmentSlot.MAINHAND).is(WAItemRegister.BOOMSHOT.get())) {GeckoLibNetwork.syncAnimation(target, this, id, 1);
            GeckoLibNetwork.syncAnimation(target, this, id2, 1);
            }
            }
